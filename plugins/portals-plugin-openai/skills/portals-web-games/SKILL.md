...
- Use `create_private_leaderboard` when the user wants a separate ranking for a tournament, class, or event. Create the board **before** sharing its link — a link naming a board that was never created silently plays the public leaderboard. Give the user the `board_url` it returns; the game code needs no change. Viewing and deleting boards happens in My Games → Settings → Private leaderboards.
